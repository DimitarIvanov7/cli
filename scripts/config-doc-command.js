...
const describeAll = (content) =>
	content
		.map((name) => definitions[name].describe())
		.join(
...
try {
	...
	if (params && params.length) {
		...
		if (!hasTag) {
			console.error(
				"WARNING: did not find config description section",
				configDoc
			);
		}
		...
		if (usage && usage.length && !hasUsageTag) {
			console.error(
				"WARNING: did not find usage description section",
				configDoc
			);
		}
